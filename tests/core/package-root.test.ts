...
async function makeTree(): Promise<{ root: string; nested: string }> {
  const root = await mkdtemp(join(tmpdir(), "specflow-pkgroot-"));
...
describe("findPackageRoot", () => {
  ...
  it("throws if no package.json exists above the start path", async () => {
    const orphan = await mkdtemp(join(tmpdir(), "specflow-orphan-"));
    const sentinel = join(orphan, "lonely.js");
    await writeFile(sentinel, "");

    expect(() => findPackageRoot(pathToFileURL(sentinel).href)).toThrow(
      /No package\.json/,
    );
  });
});
